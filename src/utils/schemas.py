...
class Keypoint(BaseModel):
    ...
    score: float = Field(..., description="Confidence score of the keypoint", ge=0, le=1)

# Schemas for object detection
class Detection(BaseModel):
    detected_class_id: int = Field(..., description="Detected class number")
    # Confidence between 0 and 1
    confidence: float = Field(..., description="Confidence score of the detection", ge=0, le=1)
...
class DetectionInferenceResponse(BaseModel):
    ...
    inference_time: float = Field(..., description="Time taken for inference in seconds", ge=0)

# Schemas for pose estimation
class Pose(BaseModel):
    detected_class_id: int = Field(..., description="Detected class id (e.g. person)")
    confidence: float = Field(..., description="Overall confidence for the pose detection", ge=0, le=1)
...
class PoseInferenceResponse(BaseModel):
    ...
    inference_time: float = Field(..., description="Inference time in seconds", ge=0)
